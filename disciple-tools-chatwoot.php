 ...
 * 1. @todo Rename the `disciple-tools-chatwoot.php file.
 * 2. @todo Refactor all occurrences of the name Disciple_Tools_Chatwoot, disciple_tools_chatwoot, disciple-tools-chatwoot, chatwoot_post_type, and "Disciple.Tools Chatwoot"
...
/**
 * Gets the instance of the `Disciple_Tools_Chatwoot` class.
 *
 * @since  0.1
 * @access public
 * @return object|bool
 */
function disciple_tools_chatwoot() {
    $disciple_tools_chatwoot_required_dt_theme_version = '1.19';
    ...
    if ( $is_theme_dt && version_compare( $version, $disciple_tools_chatwoot_required_dt_theme_version, '<' ) ) {
        add_action( 'admin_notices', 'disciple_tools_chatwoot_hook_admin_notice' );
    ...
    return Disciple_Tools_Chatwoot::instance();
}
add_action( 'after_setup_theme', 'disciple_tools_chatwoot', 20 );

//register the D.T Plugin
add_filter( 'dt_plugins', function ( $plugins ){
    $plugin_data = get_file_data( __FILE__, [ 'Version' => 'Version', 'Plugin Name' => 'Plugin Name' ], false );
    $plugins['disciple-tools-chatwoot'] = [
...
class Disciple_Tools_Chatwoot {
    ...
    private function __construct() {
         ...
         */
        // if ( strpos( dt_get_url_path(), 'metrics' ) !== false || ( $is_rest && strpos( dt_get_url_path(), 'disciple-tools-chatwoot-metrics' ) !== false ) ){
...
//        require_once( 'magic-link/magic-link-home.php' );
    ...
    public static function deactivation() {
        ...
        delete_option( 'dismissed-disciple-tools-chatwoot' );
    }
    ...
    public function i18n() {
        $domain = 'disciple-tools-chatwoot';
    ...
    public function __toString() {
        return 'disciple-tools-chatwoot';
    }
    ...
    public function __call( $method = '', $args = array() ) {
        _doing_it_wrong( 'disciple_tools_chatwoot::' . esc_html( $method ), 'Method does not exist.', '0.1' );
        unset( $method, $args );
        return null;
    }
}
...
register_activation_hook( __FILE__, [ 'Disciple_Tools_Chatwoot', 'activation' ] );
register_deactivation_hook( __FILE__, [ 'Disciple_Tools_Chatwoot', 'deactivation' ] );


if ( ! function_exists( 'disciple_tools_chatwoot_hook_admin_notice' ) ) {
    function disciple_tools_chatwoot_hook_admin_notice() {
        ...
        if ( ! get_option( 'dismissed-disciple-tools-chatwoot', false ) ) { ?>
            <div class="notice notice-error notice-disciple-tools-chatwoot is-dismissible" data-notice="disciple-tools-chatwoot">
                <p><?php echo esc_html( $message );?></p>
            </div>
            <script>
                jQuery(function($) {
                    $( document ).on( 'click', '.notice-disciple-tools-chatwoot .notice-dismiss', function () {
                        $.ajax( ajaxurl, {
                            type: 'POST',
                            data: {
                                action: 'dismissed_notice_handler',
                                type: 'disciple-tools-chatwoot',
 ...
 * @link https://github.com/DiscipleTools/disciple-tools-chatwoot/wiki/Configuring-Remote-Updating-System
 *
